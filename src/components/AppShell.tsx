...
import { LayoutDashboard, Search, Settings2, Users, Wrench, ReceiptText, Warehouse, BarChart3, Wifi } from "lucide-react";
...
export default function AppShell() {
  ...
  if (loading) {
    return <div className="flex h-screen items-center justify-center text-sm text-muted-foreground">
      Loading Uptiq session…
    </div>;
  }
  if (error) {
    return <div className="flex h-screen items-center justify-center p-6 text-center text-sm text-destructive">
      Session failed: {error}
    </div>;
  }

  const isAdmin = user?.role === "owner_admin" || user?.role === "office_manager";
    ...
    <div className="grid h-screen grid-cols-[220px_1fr] grid-rows-[44px_1fr] bg-background">
      <header className="col-span-2 flex items-center justify-between border-b border-border bg-sidebar px-4 text-sidebar-foreground">
        <div className="flex items-center gap-3">
          <div className="flex h-6 w-6 items-center justify-center rounded-sm bg-sidebar-primary text-sidebar-primary-foreground text-xs font-bold">U</div>
        ...
        <div className="flex items-center gap-3 text-xs">
          <span className="hidden md:inline text-sidebar-foreground/60">{user?.email}</span>
      ...
      <aside className="row-start-2 border-r border-border bg-sidebar text-sidebar-foreground">
        <nav className="flex flex-col gap-px p-2">
          ...
          <div className="mt-auto p-2 text-2xs text-sidebar-foreground/40 flex items-center gap-1">
            <Wifi className="h-3 w-3" /> phase-1 scaffold
          </div>
        </nav>
      </aside>
