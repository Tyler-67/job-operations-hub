...
// One component, two layouts: `compact` renders a single-column panel (embedded on the
// Dashboard between Active jobs and the Office queue); default renders the full two-column
// page layout used by the /search route.
...
export default function GlobalSearch({ compact = false }: { compact?: boolean }) {
  ...
  // Compact caps each group so the panel stays a panel; the full page shows everything.
  ...
  return (
    <div className={compact ? "flex h-full flex-col text-xs" : "flex h-full flex-col"}>
      ...
      {data && (
        compact
          ? <div className="flex-1 divide-y divide-border overflow-auto text-xs">{sections}</div>
          : <div className="grid flex-1 grid-cols-2 gap-px overflow-auto bg-border text-xs">{sections}</div>
      )}
      {!data && compact && (
