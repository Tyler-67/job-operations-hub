...
export default function InspectionDateForm({ payload }: { payload: TokenPayload }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${SUPABASE_URL}/functions/v1/forms-inspection-date`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({ token, inspection_date: date, slot }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error ?? "submit_failed");
      setDone({ date: data.inspection_date as string, slot: data.slot as Slot });
    } catch (err) {
  ...
  if (done) {
    return (
      <div className="rounded-md border border-emerald-200 bg-emerald-50 p-4 text-emerald-800">
        <p className="font-semibold">Inspection date set.</p>
        <p className="mt-1 text-sm">
          Scheduled for {done.date} ({done.slot === "1pm" ? "1:00 PM" : "9:00 AM"})
          {address ? ` at ${address}` : ""}.
        </p>
      </div>
    );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="space-y-1">
        ...
        <Input id="inspection-date" type="date" required value={date}
      ...
      <Button type="submit" disabled={submitting || !date} className="w-full">
        {submitting ? "Saving..." : "Set inspection date"}
      </Button>
    </form>
  );
}
