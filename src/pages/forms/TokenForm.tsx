...
interface Props { tokenAction: string; title: string; children?: (payload: any) => React.ReactNode }
...
export default function TokenForm({ tokenAction, title, children }: Props) {
  ...
  return (
    <div className="mx-auto max-w-md p-6">
      ...
      {status === "invalid" && <div className="rounded-sm border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">
        This link is invalid or has expired. Request a new one.
      </div>}
      {status === "ok" && (children ? children(payload) : <div className="text-sm">Form coming in Phase 2.</div>)}
    </div>
  );
}
